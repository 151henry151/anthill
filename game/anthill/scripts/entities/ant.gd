...
var world: Node
var carried: int = SimParams.BLOCK_AIR
...
func _try_grab() -> void:
	if carried != SimParams.BLOCK_AIR:
		return
	var hit := _ray_voxel_hit(4.0)
	if hit.x < -90000:
		return
	if world.get_block(hit.x, hit.y, hit.z) == SimParams.BLOCK_SAND:
		world.set_block(hit.x, hit.y, hit.z, SimParams.BLOCK_AIR)
		carried = SimParams.BLOCK_SAND


func _try_place() -> void:
	if carried != SimParams.BLOCK_SAND:
		return
	var p := global_position + Vector3.UP * 0.5 - transform.basis.z * 1.8
	var c := Vector3i(int(floor(p.x)), int(floor(p.y)), int(floor(p.z)))
	if world.get_block(c.x, c.y, c.z) == SimParams.BLOCK_AIR:
		world.set_block(c.x, c.y, c.z, SimParams.BLOCK_SAND)
		carried = SimParams.BLOCK_AIR
